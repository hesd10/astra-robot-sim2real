"""Run exactly the nine frozen stage trials; no automatic repetitions."""
import json,os,sys,time,subprocess,fcntl,hashlib
from pathlib import Path
from start_local_feedback_trial import ROOT,OUT,validate,hashes,start,preflight
from run_skill_comparison import measure
BATCH=ROOT/'reports/batches/local-feedback-001'
...
def worker():
 ...
 try:
  ...
  print('All nine trials archived; no repeats launched.',flush=True)
...
def launch():
 ...
 write(BATCH/'status.json',dict(status='prepared',completed=[],active=None,manifest_sha256=digest(OUT/'manifest.json'),worker_sha256=digest(Path(__file__)),git_head=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),created_unix=time.time(),authorized_trials=9))
 cmd=['systemd-inhibit','--what=sleep:idle','--mode=block','--why=Run nine local feedback stage trials',str(ROOT.parent/'run-local.sh'),'scripts/local_feedback_batch.py','worker']
