...
def worker():
    with (ROOT/'experiments/.iterations.lock').open('a') as lock:
        ...
        m=validate();s=read(BATCH/'status.json');assert s['active'] is None and s['status']=='prepared'
        ...
        try:
            for t in m['trials']:
                ...
                if len(s['completed'])==6:
                    after=quota(s,m);start=s['quota_at_start'];same=[x['resetsAt'] for x in after['windows']]==[x['resetsAt'] for x in start['windows']]
                    used=start['minimum_remaining_percent']-after['minimum_remaining_percent'] if same else None
                    write_json(BATCH/'six-trial-quota-calibration.json',{'completed':6,'before':start,'after':after,'same_window':same,'used_percentage_points':used,'linear_remaining12_estimate_points':max(0,used)*2 if used is not None else None,'note':'Shared integer-granularity quota includes operator activity; projection advisory only. Keep xhigh and frozen order; reserve10 enforced before trials.'})
