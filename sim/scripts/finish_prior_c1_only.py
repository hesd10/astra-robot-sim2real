"""Continue only C1 after archived B1/D1, skipping A1 as requested.
...
def check():
    ...
    if state['status'] != 'paused_after_D1' or state.get('active'):
        raise RuntimeError('Expected a clean pause after archived D1')
    if [t['label'] for t in state['completed']] != ['B1', 'D1']:
        raise RuntimeError('Expected archived B1 and D1')
...
def resume():
    with (ROOT/'experiments/.iterations.lock').open('a') as lock:
        ...
        state.setdefault('continuations', []).append({
            'requested_by_user': 'Run only C1 after D1, skip A1, then stop for protocol review',
        ...
        try:
            ...
            for label in ['C1']:
                ...
                state['pending_order'] = [x for x in ['C1'] if x not in {t['label'] for t in state['completed']}]
                write_json(BATCH/'status.json',state)
            ...
            write_json(BATCH/'status.json',state)
            print('Requested trials complete: B1, D1, C1. A1 skipped; stop for protocol review.',flush=True)
...
def detach():
    ...
    directory = ROOT/'reports/studies'/NAME/'block-1-c1-only'
    directory.mkdir(parents=True,exist_ok=False)
    command = [inhibitor,'--what=sleep:idle','--mode=block',
        '--why=Astra remaining first-block prior experiments',
        str(ROOT.parent/'run-local.sh'),'scripts/finish_prior_c1_only.py','--worker']
    ...
    receipt = {'pid':process.pid,'started_unix':time.time(),'command':command,
        'retained_trials':['B1','D1'],'remaining_order':['C1'],
...
if __name__ == '__main__':
    ...
    if args.check_only:
        check()
        print('B1 archive and frozen inputs verified; C1 ready; A1 skipped. No inference started.')
    elif args.worker:
        resume()
    else:
        detach()
