"""Run only 12 new C/R trials, with pair reviews and a final-analysis stop.
...
from prepare_experience_reflection import ROOT, OUT, validate, digest
from account_preflight import preflight
import run_prior_comparison as legacy

BATCH=ROOT/'reports/batches/experience-reflection-001'
...
def aggregate(s):
    ...
    text=['# 单源精简/复盘对照进度','',f"状态：{s['status']}；已归档 {len(rows)}/12。",'',
    ...
    with (ROOT/'experiments/.iterations.lock').open('a') as lock:
        ...
        try:
            for index,t in enumerate(m['trials']):
                ...
                cmd=[sys.executable,'-u',str(ROOT/'scripts/start_reflection_trial.py'),t['run']]
...
def launch():
    ...
    save(s)
    command=['systemd-inhibit','--what=sleep:idle','--mode=block','--why=Run XLeRobot single-source experience comparisons',str(ROOT.parent/'run-local.sh'),'scripts/reflection_batch.py','worker']
