"""Continue D1/A1/C1 after the explicitly reviewed, archived B1.
...
def check():
    ...
    if state['status'] != 'paused_after_B1' or state.get('active'):
        raise RuntimeError('Expected a clean pause after archived B1')
    if [t['label'] for t in state['completed']] != ['B1']:
        raise RuntimeError('Expected exactly one archived B1')
    ...
    if state['pending_order'] != ['D1', 'A1', 'C1']:
        raise RuntimeError('Unexpected remaining conditions')
    for label in state['pending_order']:
    ...
    verified = summarize(ROOT/'experiments/prior-001-B1', manifest['variants']['B'])
    if verified != state['completed'][0]['result']:
        raise RuntimeError('Archived B1 metrics changed')
...
def resume():
    with (ROOT/'experiments/.iterations.lock').open('a') as lock:
        ...
        state.setdefault('continuations', []).append({
            'requested_by_user': 'Continue remaining first-block conditions without pausing',
        ...
        try:
            ...
            for label in ['D1','A1','C1']:
                ...
                print('Starting '+name+'; B1 retained, frozen inputs unchanged.', flush=True)
                ...
                state['pending_order'] = [x for x in ['D1','A1','C1'] if x not in {t['label'] for t in state['completed']}]
                write_json(BATCH/'status.json',state)
            ...
            write_json(BATCH/'status.json',state)
            print('First block complete: B1, D1, A1, C1. No later block started.',flush=True)
...
def detach():
    ...
    directory = ROOT/'reports/studies'/NAME/'block-1-continuation'
    directory.mkdir(parents=True,exist_ok=False)
    command = [inhibitor,'--what=sleep:idle','--mode=block',
        '--why=Astra remaining first-block prior experiments',
        str(ROOT.parent/'run-local.sh'),'scripts/resume_prior_batch.py','--worker']
    ...
    receipt = {'pid':process.pid,'started_unix':time.time(),'command':command,
        'retained_trial':'B1','remaining_order':['D1','A1','C1'],
...
if __name__ == '__main__':
    ...
    if args.check_only:
        check()
        print('B1 archive and frozen inputs verified; D1/A1/C1 ready. No inference started.')
    elif args.worker:
        resume()
    else:
        detach()
