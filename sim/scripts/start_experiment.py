"""Fresh native Codex attempt, private live camera, full replay export."""
import argparse
import json
...
from sim_env.transfer import copy_clean
from sim_env.setup import load_setup


def start(name, *, prepare_only=False, max_seconds=3600., reasoning='xhigh', skill=None,
    ...
    subject=prepare(run/'subject')
    private=run/'private';private.mkdir()
    if skill:
        transfer=copy_clean(skill,subject/'skill')
        (private/'transfer-audit.json').write_text(json.dumps(transfer,indent=2)+'\n')
        git=['git','--git-dir='+str(subject/'.git'),'--work-tree='+str(subject)]
        subprocess.run(git+['add','skill'],check=True)
        subprocess.run(git+['commit','-qm','Import audited generic skill only'],check=True)
    configuration={'run':name,'max_seconds':max_seconds,'max_requests':None,'reasoning':reasoning,
                   ...
                   'skill_transfer':'mechanically screened skill only' if skill else 'none; fresh first-run baseline','ready':True}
    ...
    service_args=[sys.executable,'-m','sim_env.service','--socket-path',str(sock),
    ...
    with (private/'service.log').open('w') as log:
        ...
        try:
            ...
            result=run_agent(subject,sock,private,max_seconds=max_seconds,reasoning=reasoning)
...
if __name__=='__main__':
    ...
    p.add_argument('--skill',help='operator-selected cleaned skill directory only; never copy a prior workspace')
