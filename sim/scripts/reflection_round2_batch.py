"""Run 18 new E3/C/R trials, with pair reviews and a final-analysis stop.
Derived from skill_evaluation_batch.py; old study files are unchanged.
PAUSE_REQUEST.json prevents subsequent starts and survives heartbeat changes.
"""
import argparse
...
BATCH=ROOT/'reports/batches/experience-reflection-001-round2'
...
def aggregate(s):
    ...
    write(BATCH/'summary.json',{'conditions':conditions,'trials':rows,'historical_baselines':read(OUT/'baselines.json'),'note':'Success proxy still requires image review. Paired trial records, not pooled means, determine per-point comparison.'})
    text=['# 单源精简/复盘对照进度','',f"状态：{s['status']}；已归档 {len(rows)}/18。",'',
...
def pair_gate(s,m):
    pair=s['completed'][-3:]
    assert len(pair)==3 and len({t['point'] for t in pair})==1 and len({t['repeat'] for t in pair})==1 and {t['condition'] for t in pair}=={'E3','C','R'}
    ...
    pending={'pair':key,'runs':[r['run'] for r in pair],'manifest_sha256':s['manifest_sha256'],
             'review_file':str(path),'trial_results':pair,'historical_baseline':next(b for b in read(OUT/'baselines.json') if b['point']==pair[0]['point']),
             'review_fields':['selected historical images and guide usage','public actions and decisions','fresh visual outcome','faults','same-point historical E3 / C / R time and token differences'],
...
def pause_if_requested(s):
    if not (BATCH/'PAUSE_REQUEST.json').exists():return
    previous=s['status']
    while (BATCH/'PAUSE_REQUEST.json').exists():
        s.update(status='paused_by_user',last_checked_unix=time.time());save(s);time.sleep(10)
    s['status']=previous;save(s)
    ...
    with (ROOT/'experiments/.iterations.lock').open('a') as lock:
        ...
        try:
            for index,t in enumerate(m['trials']):
                ...
                cmd=[sys.executable,'-u',str(ROOT/'scripts/start_reflection_round2_trial.py'),t['run']]
...
def launch():
    ...
    first_status=read(ROOT/'reports/batches/experience-reflection-001/status.json')
    assert len(first_status['completed'])==12 and len(first_status.get('reviewed_pairs',[]))==6
    s['account_fingerprint']=first_status['account_fingerprint']
    save(s)
    command=['systemd-inhibit','--what=sleep:idle','--mode=block','--why=Run XLeRobot single-source experience comparisons',str(ROOT.parent/'run-local.sh'),'scripts/reflection_round2_batch.py','worker']
