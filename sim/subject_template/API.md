...
Use a fresh workspace/conversation. If no skill/ is supplied, start from scratch.
Current-run Git history is available for the final audit; imported prior history
is prohibited. No XML/MJCF/URDF, kinematic parameters, existing IK/navigation/press
controllers, or scene reconstructions may be obtained from outside this attempt.
You may infer relationships and write controllers from current-run observations.
On contamination, stop safely and call finish with outcome contamination.
Stable joint telemetry and self-declared success alone do not establish task
completion; distinguish observations, assumptions, partial results and limitations.

After finish, audit skill/SKILL.md and reusable code before the final commit.
Only general procedures, qualitative lessons and generic parameterized code may
transfer. Do not transfer images, recorded encoder counts, angles, poses,
coordinates, scene reconstructions, calibration tables, trajectories, replays or
encoded equivalents. Do not transfer .git, run controllers, evidence, logs or
reports; reusable generic code must first be explicitly cleaned into skill/.
