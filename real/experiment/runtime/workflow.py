...
class Workflow:
    def __init__(self, log_dir, agent_file=None):
        ...
        self.operator = Tail(self.log_dir / 'operator-events.jsonl')
    ...
    def poll(self):
        ...
        for row in self.operator.read():
            if row.get('kind')=='request':
                rows.append(('robot',dict(event='request',monotonic=row['unix']-time.time()+time.monotonic(),request=row['request'],accepted=True,reply=row.get('result'))))
